class LazyPolynom(object):
    """
    the LazyPolynom module will represent a Polynom with simple operations like add,multiply and compose of other polynoms in time complexity of O(1), then it will be possible to evaluate the polynom in time complexity of O(n). 
    all this will be fairly easy and possible using lambda expressions & closures features in python :).
    it is lazy because it postpone the need to evaluate & manipulate the polynom in order to do manipulations on it.
    """
    def __init__(self):
